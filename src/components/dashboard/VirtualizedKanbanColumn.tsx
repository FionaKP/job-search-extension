...
const CARD_HEIGHT = 110; // Approximate height of a posting card in kanban view
...
const CardRow = memo(function CardRow({
  ...
  return (
    <div style={{ ...style, paddingBottom: CARD_SPACING }}>
...
export const VirtualizedKanbanColumn = memo(function VirtualizedKanbanColumn({
  ...
  const addButtonHeight = onAdd ? 40 : 0;
  const listHeight = columnHeight - headerHeight - addButtonHeight - 16; // 16 for padding

  const checkScroll = useCallback(() => {
    const list = listRef.current;
    if (!list) return;

    const totalHeight = postings.length * (CARD_HEIGHT + CARD_SPACING);
    const visibleHeight = listHeight;

    setHasMoreBelow(totalHeight > visibleHeight);
  }, [postings.length, listHeight]);
  ...
  // Use virtualization only for columns with many items
  const useVirtualization = postings.length > 20;
        ...
        {useVirtualization ? (
          ...
          <div className="absolute inset-0 p-2">
            <List
              ...
              itemSize={CARD_HEIGHT + CARD_SPACING}
              itemData={itemData}
              overscanCount={3}
          ...
          /* Regular rendering for few items */
          <div className="absolute inset-0 space-y-2 overflow-y-auto p-2 scrollbar-subtle">
